...
def customer_ids_from_events(events):
    customer_ids = set()
...
def fetch_clickhouse_customers(database):
    ...
    return {
        row["customer_alternate_key"]: row
        for row in rows
    }
...
def fetch_clickhouse_geographies(database):
    ...
    return {
        normalized_geography_key(row): row
        for row in rows
    }
...
def build_customer_changes(
    ...
    for customer_id in customer_ids:
        ...
        geography_identity = normalized_geography_key(
            geography_source
        )

        existing_geography = existing_geographies.get(
            geography_identity
        )
...
def complete_batch(cursor, old_watermark, new_watermark):
    ...
    if cursor.rowcount != 1:
        raise RuntimeError(
            "Pipeline watermark update failed"
        )
    ...
    try:
        with connection.cursor(
            ...
            if not events:
                connection.rollback()
                print(
                    f"NO PENDING EVENTS watermark={watermark}",
                    flush=True,
                )
                return
            ...
            customer_ids = customer_ids_from_events(events)
            ...
            existing_customers = fetch_clickhouse_customers(
                database
            )

            existing_geographies = fetch_clickhouse_geographies(
                database
            )
